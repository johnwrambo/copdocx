...
`store.promoteBookInToLead(input)` (**0.27.0**) is Book-in Save as an express lead form. It mints or reuses a **person** and files a **committed** lead with `caseRole: "DETAINEE"` (in custody). Reuse order: `leadId`, then `personId`, then A-Number digits. Identity overlay (name, sex, DOB, citizenship, A-Number, disposition, status) does **not** copy RAP and does not wipe arrests / warrants / baseball cards. Empty identity (no name, A-Number, or existing id) is rejected. Packet store stays `alien-book-in.saved-records.v1`; this is a cross-store write on Book-in **Save** only (not autosave). Encounter `subjects[]` gets `personId` and `leadId`.
...
`store.associations{}` (**0.53.0**) is the first-class relational layer. `createAssociation` (`copdocx.association.v1`) is the record: canonical A6 `from`/`to`/`reason`, optional `occupancy` / `validFrom` / `validTo`, provenance `source`, `junked`. Investigation `links[]` **cite** `associationId`; they are this map’s edges, not a second fact. `store.upsertAssociation` reuses the same ends+reason (symmetric types match either order). `associationsFor(type, id)` lists both ends. Spawn copies new `linkId`s and the same `associationId`. Remove from wall drops the citation, not the fact. Reuse-on-type retargets association ends. Delete an unreferenced object drops its associations. A6 is the only catalog (`CUSTOMER_OF` PERSON→BUSINESS added). `registeredOwnerName` stays a title-print string. Nested `person.locations[]` / `lead.vehicles[]` are dual-written from associations when the person is a case subject (**0.56.0**) so case map and list city stay filled. Occupancy dates dual-write from the association (**0.59.0**). Form-collected nested copies are kept and merged, not replaced. **0.54.0** `store.associateInvestigationPerson` is the Card composer: type a name, reuse or mint, place the person, cite the association. **0.55.0** `store.associateInvestigationObject` does the same for Vehicle / Location / Business / Entity (plate, street, or name). Host card stays focused. Empty `registeredOwnerName` is filled once on RO. × (`disconnectInvestigationAssociation`) drops this wall’s citation only. **0.57.0** `store.associateCaseObject` is the same constructor on a case subject (cites `lead.links[]`). **0.58.0** the case Associations tile is that composer live (type a name / plate / street, Enter). × (`dropAssociation`) deletes the fact and uncite every case/wall `links[]` that pointed at it. Objects stay. Leftover OTHER × is `removeCaseLink`. See [association-plan.md](association-plan.md).
...
`createEncounterSubject`: `{ personId, leadId, bookinRecordId, lastName, firstName, alienNumber, encounterRole }`. `encounterRole` is `"TARGET"` | `"COLLATERAL"` | `""` for **this** field encounter (not a person attribute). Subjects are Book-in records tagged `encounterId` in `alien-book-in.saved-records.v1`. Book-in Save copies a summary onto `subjects[]` and files a DETAINEE lead. Do not merge the Book-in store.
