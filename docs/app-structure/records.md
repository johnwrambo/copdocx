...
- Evidence files are on the encounter. Narrative tab embeds the I-213 engine (`narrative.html?encounterId=&embed=1`) full-bleed under the banner and tabs — no status line or supervisor-summary note. **Pop out** uses `window.open()` so the I-213 draft sits in its own window while left-side elements scroll. **Dock** (or closing that window) returns the split pane. Review Confirm locks and writes the snapshot. Unlock requires a reason.
