...
- **Add encounter** mints a transient `encounterId` in memory. A blank page does not write or change the URL. The first meaningful draft write (or an explicit workflow action that needs an id) persists it and `replaceState`s `?id=`. The ID field is readonly.
- Commit requires `startedAt`, then **stays** on `encounter-form.html?id=` (the form is the home until a view exists).
- **Add subject** quiet-saves, then `bookin.html?encounterId=`. Book-in defaults Target and copies the encounter stop date/time into Book-in date/time and arrest time. Chrome: **Save to encounter** (files the packet, `encounter.subjects[]`, and canonical Arrest, then returns), **Add another book-in** (saves this subject and blanks the form for the next), **Cancel book-in** (returns without attaching an unfiled draft). The encounter subjects table lists those packets.
- Book-in with `?encounterId=` is a satellite: loud **Save** files/reuses a Person + DETAINEE lead and upserts its canonical Arrest, writes `personId` / `leadId` / `arrestId` on the packet and `encounter.subjects[]`, then returns to the encounter. **Add another subject** quiet-saves a named packet (does not file a lead) and blanks the form. **Load from cases** fills committed-case fields that exist on Book-in (no middle name). Autosave does not file a case.
...
- **Delete** on the list and the form confirms, then `store.deleteEncounter`. Book-in packets on that encounter are unlinked (`encounterId` cleared); cases and arrests stay. After delete from the form, go to `encounter.html`.
- **Complete** asks to confirm, then `store.saveEncounter(..., { mode: "complete" })` writes `completed` (`copdocx.encounter-snapshot.v1`) and sets `meta.markedComplete`. The form locks. The list **Completed** filter shows those encounters (Open, not Edit). Autosave/Save/Add subject do not run on a completed encounter. Map Encounters reads that snapshot. Book-in promotion copies the stop lat/long onto the canonical Arrest when present.
