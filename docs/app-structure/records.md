...
Operations (**0.64.0**): list `operations.html`, plan `operation-form.html`, issued view `operation.html`. Draft → form. Filed → view. **Import targets** pulls filed cases with a current place or vehicle. **Import cell** takes 2–4 roster officers (assignment roles, availability from duty/shifts/other ops). One cell per target. The operation map plots those pins (live while planning; freeze on issue). **Generate brief** later opens `operation-brief.html` (operation sheet nesting Target sheets). Quiet draft on form change once name or dates exist; Add does not write an empty row. Does not write `officer.duty`.
