...
- Evidence files are on the encounter. Narrative tab mounts the Build 9 I-213 engine in-page (`#narrativeEngineHost`, `COPDoc.narratives.bootWorkspace`) with the fact pane (filter / collapse). No iframe. Live mode seeds and hides fields the stop already owns (origin when mapped, location type, vehicle disposition, conduct/flight/force, enforcement/outcome, booked closing facts). **Advanced** reveals those rows. **Pop out** uses `window.open()` for the draft. Review Confirm locks and writes the snapshot. Unlock requires a reason.
