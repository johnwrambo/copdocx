...
(function (global) {
  ...
  function loadPreference(method) {
    try { return root.repositories.viewState[method](); }
    catch (err) { return null; }
  }

  function savePreference(method, value) {
    try { root.repositories.viewState[method](value); }
    catch (err) {}
  }
  ...
  function loadPrefs() {
    var layers = loadPreference("loadMapLayers");
    ...
    var stored = loadPreference("loadMapIcons");
  ...
  function saveLayers() {
    savePreference("saveMapLayers", { visible: visible });
  }
  ...
  function saveIcons() {
    ...
    savePreference("saveMapIcons", icons);
  }
  ...
  function formatAddress(location) {
    return root.projections.map.formatAddress(location);
  }

  function hasCoords(lat, lng) {
    return root.projections.map.hasCoords(lat, lng);
  }

  function parseCoords(text) {
    return root.projections.map.parseCoords(text);
  }
  ...
  function subjectFor(snapshot) {
    return root.projections.map.subjectFor(snapshot);
  }
  ...
  function vehicleSummary(vehicle) {
    return root.projections.map.vehicleSummary(vehicle);
  }

  function pushOwner(list, type, id) {
    return root.projections.map.pushOwner(list, type, id);
  }
  ...
  function pinFromEncounter(encounter, storeState) {
    return root.projections.map.encounterPin(encounter, storeState);
  }

  function hydrateMapLocation(loc, storeState) {
    return root.projections.map.hydrateLocation(loc, storeState);
  }

  function collectEncounters(storeState) {
    catalog.encounters = root.projections.map.completedEncounters(storeState);
  }
  ...
  function collectOfficers() {
    catalog.officers = [];
    var admin;
    try { admin = root.repositories.admin.readSnapshot(); }
    catch (err) { admin = { officers: [] }; }
    (admin.officers || []).forEach(function (officer) {
  ...
  function encounterFlags(encounter, source) {
    return root.projections.map.encounterFlags(encounter, source);
  }
  ...
  function arrestHeatPoints() {
    return root.projections.map.heatPoints(catalog.arrests);
  }

  function heatCellSizeDeg(map) {
    return root.projections.map.heatCellSize(map && typeof map.getZoom === "function" ? map.getZoom() : 10);
  }

  function computeHeatPeaksGeo(points, cellDeg) {
    return root.projections.map.heatPeaks(points, cellDeg);
  }
