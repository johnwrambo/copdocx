/**
 * Transfer application service: portable record transforms, graph closure,
 * detached import planning and consistent export capture. UI and persistence
 * are supplied as explicit capabilities; this module never reads browser DOM.
 */
(function (global) {
  "use strict";
  var app = global.COPDoc = global.COPDoc || {};
  var application = app.application = app.application || {};
  application.createTransfer = function (dependencies) {
  ...
  function storageApi() { return importStorage || dependencies.repository.channel(); }
  ...
  function isCommitted(row) {
    ...
    if (dependencies.getModel() && typeof dependencies.getModel().isCommitted === "function") {
      return dependencies.getModel().isCommitted(row);
    }
  ...
  function collectSupportState() {
    ...
    var basemap = "";
    if (dependencies.repository.hasStorage()) {
      try {
        basemap = dependencies.repository.channel().getItem(MAP_BASEMAP_KEY) || "";
      } catch (error) {
        basemap = "";
      }
    }
  ...
  function applySupportState(parsed) {
    ...
    if (parsed.map && typeof parsed.map === "object") {
      ...
      if (parsed.map.basemap && dependencies.repository.hasStorage()) {
        try {
  ...
  function refreshBookInCardsForExport(packets) {
    ...
    var baseball = dependencies.getBaseball();
  ...
  function csvEscape(value) {
    if (dependencies.getModel() && typeof dependencies.getModel().csvCell === "function") {
      return dependencies.getModel().csvCell(value);
    }
  ...
  function parseTransfer(text, options) {
    ...
    var sharedDecoder = dependencies.getDecoder();
    ...
    if (data.format === "alien-book-in-records") {
      var decoder = dependencies.getDecoder();
  ...
  function prepareObjectContractImport(parsed, types, prepared) {
    ...
    if (error) return { ok: false, error: error };
    var store = dependencies.getModel() && dependencies.getModel().store;
  ...
  function canonicalBookInStore() {
    var model = dependencies.getModel();
  ...
  function captureImportStorage() {
    ...
    return dependencies.repository.capture(defaults, config && config.storageEntries || []);
  }
  ...
  function applyImport(parsed, types, options) {
    ...
    var workflow = dependencies.getImportWorkflow();
  ...
  /** Capture once, await Media, then verify the source bytes before delivery. */
  async function buildExport(types, from, to, options) {
    options = options || {};
    var exportBefore = captureImportStorage();
    var bundle = collectExport(types, from, to);
    var count = exportCount(bundle);
    if (!count) return { bundle: bundle, count: 0, mediaCount: 0 };
    if (options.includeMedia) {
      if (typeof options.exportMedia !== "function") throw new Error("Photo and file storage could not be loaded. Export was not completed.");
      bundle.media = await options.exportMedia();
    ...
    if (!dependencies.repository.sourcesMatch(exportBefore, [LEAD_KEY, ADMIN_KEY, BOOKIN_KEY])) throw new Error("Workspace changed while collecting export data. Export again to capture one consistent version.");
    return { bundle: bundle, count: count, mediaCount: bundle.media ? bundle.media.length : 0 };
  }
  ...
  return {
    ...
    inRange: inRange,
    jsonEqual: jsonEqual,
    canonicalBookInStore: canonicalBookInStore,
    typeCsv: typeCsv,
