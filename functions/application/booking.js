/** Recoverable booking application service. Persistence and domain capabilities are injected. */
(function (global) {
  "use strict";
  var app = global.COPDoc = global.COPDoc || {};
  var application = app.application = app.application || {};
  application.createBooking = function (deps) {
  ...
  function key(id, fallback) { return deps.getConfig() && deps.getConfig().storageKey(id) || fallback; }
  ...
  function read(k, fallback, predicate) {
    var raw = deps.storage.read("localStorage", k);
    var value;
  ...
  function write(k, previous, value) {
    var writable = deps.getImportWorkflow() && deps.getImportWorkflow().assertWritable();
    if (writable && !writable.ok) fail(writable.error);
    if (deps.storage.read("localStorage", k) !== previous) fail("Booking data changed in another window. Resume after reviewing the change.");
    var next = JSON.stringify(value);
    deps.storage.write("localStorage", k, next);
    if (deps.storage.read("localStorage", k) !== next) fail("The booking write could not be verified.");
    return next;
  }
  function journal() {
  ...
  function subjectId(row) { return text(deps.getModel().encounterSubjectId(row)); }
  function subjectBooking(row) { return text(deps.getModel().encounterSubjectBookingId(row)); }
  function store() {
    if (!deps.getModel() || !deps.getModel().store || !deps.getModel().store.resolveBookInBooking) fail("The booking store is unavailable.");
    return deps.getModel().store;
  }
  function sources() {
  ...
  function context(packet, src, allowLocked, reservation) {
    ...
    if (!matches.length && reservation) {
      if (reservation === true && !sid) {
        ...
        reserved = deps.getModel().createEncounterSubject({ subjectId: newId("sub_"), encounterId: encId,
    ...
    packet.subjectRole = deps.getModel().encounterSubjectRole(subject);
    packet.encounterRole = packet.subjectRole;
    packet.vehiclePosition = deps.getModel().encounterSubjectOccupantRole(subject);
  ...
  function linkSubject(t, packet) {
    ...
    checked(src.store.updateEncounter(packet.encounterId, function (enc) {
      ...
      return enc;
    }, { mode: deps.getModel().isCommitted(ctx.encounter) ? "commit" : "draft" }), "Could not link the Encounter subject.");
  }
  ...
  function runTransaction(t) {
    ...
    try {
      ...
      if (ctx.reservedSubject) {
        ...
        }, { mode: deps.getModel().isCommitted(ctx.encounter) ? "commit" : "draft" }), "Could not create the reserved Encounter subject.");
      }
      ...
      if (ctx.subject && text(ctx.subject.arrestingOfficerId)) {
        if (!deps.getOfficers() || !deps.getOfficers().recordFieldArrest) fail("The officer booking projection is unavailable.");
        checked(deps.getOfficers().recordFieldArrest(ctx.subject.arrestingOfficerId, { arrestId: packet.arrestId, encounterId: packet.encounterId, subjectId: packet.subjectId, personId: packet.personId, bookingId: packet.id, bookedAt: t.createdAt }), "Could not update the officer arrest projection.");
      }
  ...
  function newId(prefix) {
    return prefix + (deps.getCrypto() && deps.getCrypto().randomUUID ? deps.getCrypto().randomUUID() : Date.now().toString(36) + "-" + Math.random().toString(36).slice(2));
  }
  ...
  function serialized(action) {
    var run = function () {
      if (deps.getLocks() && typeof deps.getLocks().request === "function") {
        return deps.getLocks().request("copdocx.booking-workflow.v1", { mode: "exclusive" }, action);
      }
      return action();
    };
  ...
  function runVoid(t) {
    ...
    try {
      ...
      (src.admin.officers || []).forEach(function (officer) {
        ...
        if (!deps.getOfficers() || !deps.getOfficers().voidFieldArrest) fail("The officer void projection is unavailable.");
        checked(deps.getOfficers().voidFieldArrest(text(officer.id || officer.officerId), voidInput(t)), "Could not void the officer statistic.");
      });
