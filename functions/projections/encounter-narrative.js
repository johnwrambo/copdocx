/**
 * Encounter → Build 9 projection. Dependencies are supplied by the composition adapter.
 */
(function (global) {
  "use strict";

  /** Each projection instance receives its data readers; it owns no browser state. */
  function createEncounterNarrative(dependencies) {
    var deps = dependencies || {};
    function modelOf() { return typeof deps.getModel === "function" ? deps.getModel() : deps.model; }
    function readSource(name, fallback, unavailable) {
      try {
        var value = deps[name]();
        if (Array.isArray(fallback) ? !Array.isArray(value) : !value || typeof value !== "object" || Array.isArray(value)) {
          throw new Error("Narrative source has an invalid shape.");
        }
        return value;
      } catch (error) {
        if (unavailable) unavailable.push({ readBookins: "bookin", readAdmin: "admin", readSettings: "settings" }[name]);
        return fallback;
      }
    }
    function bookinRecords(unavailable) { return readSource("readBookins", [], unavailable); }
    function readAdmin(unavailable) { return readSource("readAdmin", {}, unavailable); }
    function readSettings(unavailable) { return readSource("readSettings", {}, unavailable); }
  ...
  var api = { createEncounterNarrative: createEncounterNarrative };
  if (typeof module === "object" && module.exports) module.exports = api;
  global.COPDoc = global.COPDoc || {};
  global.COPDoc.projections = global.COPDoc.projections || {};
  global.COPDoc.projections.createEncounterNarrative = createEncounterNarrative;
